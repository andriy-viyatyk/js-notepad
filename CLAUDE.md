...
| `board-manifest.json` identity file (read/ensure; a folder is a board iff it carries one; also the Custom Editor axis — `fileMasks`/`editorPriority`/`editorName` + `normalizeFileMasks`/`matchesFileMask`/`getBoardEditorAssociation`) | `/src/renderer/editors/board/board-manifest.ts` |
