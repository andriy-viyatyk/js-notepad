...
| Shared text-host footer (`script` toggle · provider icon · encoding label; extracted from `TextChrome` so it's shared by built-in text editors and content-host boards via `BoardEditorView`) | `/src/renderer/editors/base/ContentHostFooter.tsx` |
