...
| `board-manifest.json` identity file (read/ensure; a folder is a board iff it carries one; also the Custom Editor axis — `fileMasks`/`editorPriority`/`editorName`/`editorKind` (`"simple"` \| `"content-host"`) + `normalizeFileMasks`/`matchesFileMask`/`getBoardEditorAssociation`) | `/src/renderer/editors/board/board-manifest.ts` |
| Custom-editor registry (reactive `mask → trusted board` map over `boardTrust`; `board-editor:<root>` virtual ids via `boardEditorId`/`parseBoardEditorId`; `resolveEditorIdForFile` merges built-in + board candidates at file-open; `isBoardEditorId` for MCP/automation board detection) | `/src/renderer/editors/board/custom-editor-registry.ts` |
...
| Board host (locked-down cross-origin `<iframe src="board://<host>/index.html?v=<boardId>">`, no `sandbox` attr; the per-mount `?v=<boardId>` nonce uniquely identifies this tab's frame for CDP automation — passed to `registerBoardFrame`; brokers the one-time `MessagePort` handshake into the frame; resets the board's `ui.log` per load; routes shim `board:interact`/`board:error` posts) | `/src/renderer/editors/board/BoardWebview.tsx` |
...
| Board bridge shim (browser IIFE inlined into served board HTML; rebuilds `window.persephone` over the `MessagePort` — queue-then-flush, `execute` streaming, dialogs, files, theme, `setBoardBusy`/`getBoardBusy`/`getJobs`; posts `board:interact`/`board:error`/`board:busy`/`securitypolicyviolation`/`window.onerror` to the host frame; **content-host boards** — `persephone.host.*` (`getContent`/`setContent`/`onContentChange`/`getLanguage`/`save`) over the renderer-pushed `host:content` message + `board:setContent`/`board:save` back, plus an automatic `window`-level Ctrl+S that posts `board:save` unless a board handler `preventDefault`s it) | `/src/board-shim.ts` |
