...
//! and the model provisioner (US-656) that downloads/verifies the configured embedding-model
//! files into a local cache with HTTP range-resume and sha256 verification.
//! No embeddings (US-657) yet — `chunks_vec` is created but empty, so `wiki_search` is FTS-only.
//!
