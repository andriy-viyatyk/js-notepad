 ...
 *  global capture handler for OS files), then the `dataTransfer` MIME. Accepts internal and OS drag events.
 *  The one accessor every drop target uses, so internal trait drags and OS file drags read identically. */
