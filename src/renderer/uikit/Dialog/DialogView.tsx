...
interface DialogCommitSignalProps {
    onCommit: () => void;
}

/** Signals after every nested React commit so the focus pass sees mounted children. */
function DialogCommitSignal({ onCommit }: DialogCommitSignalProps): null {
    useLayoutEffect(() => {
        onCommit();
    });
    return null;
