...
export class DownloadButtonView extends VanillaView<Record<string, never>> {
    ...
    private readonly buttonElementRef = (element: HTMLButtonElement | null): void => { this.buttonElement = element; };
    ...
    public constructor() {
        ...
        this.button = this.child(new IconButtonView({ name: "toolbar-downloads", ref: this.buttonElementRef, size: "sm", title: "Downloads", icon: "download", onClick: this.handleClick }));
    }
    ...
    protected onDispose(): void { this.buttonElement = null; }
    ...
    private readonly handleClick = (): void => { if (!this.buttonElement) return; if (isDownloadsPopupOpen()) closeDownloadsPopup(); else showDownloadsPopup(this.buttonElement); };
}
