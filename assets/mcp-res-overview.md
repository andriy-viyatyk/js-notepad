...
- **A browser page contains inner tabs.** One `browser-view` page hosts multiple browser tabs
  ...
  sessions. `browser_*` tools drive them, Playwright-style.
...
| Open a web page | `open_url` → returns `pageId` | — |
...
1. **Target explicitly.** Capture `pageId` from tool results (`open_url`, `open_board`,
...
2. **Verify, don't assume.** After creating content, check it: `get_page_content` for text,
   `browser_snapshot({ pageId: "app" })` to see whether an editor rendered or shows an error.
