...
- App-window automation: the `browser_*` tools also drive Persephone's own UI with `pageId: "app"` (snapshot/click/type/press_key/screenshot/evaluate the tab strip, sidebar, dialogs, and active editor) — explicit-only (never resolved by fallback), no registration (the calling window's own webContents is the target); see [browser-editor.md](browser-editor.md) "App-Window Target".
