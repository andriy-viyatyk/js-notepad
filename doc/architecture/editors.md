...
- `findCompatibleEditors()` returns the file's natural built-in id plus this board, **without** the `isPlainLocalPath` gate — content-host boards edit `https://` / archive / encrypted files too. On an **untitled page** (no file path yet) it falls back to the host/page **title** as the file name, so the natural built-in editor stays resolvable and the user can switch back after renaming a fresh page to a matching name (e.g. `untitled` → `diagram.drawio`).
...
**The content bridge — `persephone.host.*`.** The host is a renderer-side object, so its bridge rides the board↔host-renderer channel (`postMessage`), not the main `MessagePort`. `BoardWebview` subscribes to `host.state` and pushes `{ __persephone: "host:content", content, language }` into the iframe after the frame loads and on every change (echo-guarded against the board's own writes); the board posts `board:setContent` / `board:save` back. The shim exposes `persephone.host.getContent()` (settle-once + await-any-time), `setContent()`, `onContentChange(cb)`, `getLanguage()`, and `save()`, gated by a `contentHost` handshake flag (they reject/no-op on a plain board). **Automatic Ctrl+S:** the shim registers a `window`-level keydown handler (before any author script) that posts `board:save` → `host.io.saveFile()` unless a board handler already called `preventDefault()` — so saving works with zero board code. The DrawIO viewer board (read-only; edits happen in Monaco after a host transfer) is the reference implementation.
