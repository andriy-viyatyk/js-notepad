...
All editors flow through the same path — there is no longer a content-view branching point that wraps text-bearing editors inside `TextEditorView`. An editor may expose a React `Component` or a framework-free `View`; `AsyncEditorView` mounts the vanilla arm directly when present. Native text editors compose `TextChromeView`, while no-host editors that need shared page controls compose `PageToolbarView`. `EditorToolbar` and `ContentHostFooter` remain thin React compatibility faces for callers that still live in React.
