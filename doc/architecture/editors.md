...
**File delivery.** The associated file path rides `ILinkData.filePath` (persisted, never baked into the `persephone-board://` URL) → `BoardEditorState.filePath` → `BoardEditorModel.currentFilePath()` → `BoardPortInitMsg.filePath`, and reaches the board as the async bridge method `persephone.getFilePath()`. The board reads/writes the file with the existing top-level `persephone.readFile()` / `writeFile()` — no Persephone content pipe is involved (the direct-`filePath` case). The DrawIO viewer board is the reference implementation.
