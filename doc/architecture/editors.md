...
All editors flow through the same path — there is no longer a content-view branching point that wraps text-bearing editors inside `TextEditorView`. An editor may expose a React `Component` or a framework-free `View`; `AsyncEditorView` mounts the vanilla arm directly when present. Native text editors compose `TextChromeView`, while no-host editors that need shared page controls compose `PageToolbarView`. `PageToolbar`, `EditorToolbar`, and `ContentHostFooter` remain thin React compatibility faces for callers that still live in React.

**Error protection:** React editor components are wrapped in `EditorErrorBoundary` (`/src/renderer/ui/app/EditorErrorBoundary.tsx`); if a React body inside a native editor view remains an island, that body is wrapped at the editor's `TextChromeView` slot. The boundary catches React render errors and displays the error message + stack trace in the tab instead of crashing the application. Native view construction and mount failures are handled by `AsyncEditorView`. `EditorErrorBoundary` is a React class component (required for `getDerivedStateFromError`).
...
**Host-state-driven switch offers:** an editor's `accepts(input)` receives the candidate `input.host`, so a switch can be offered conditionally on host state — not just file type. The `file-diff` editor uses this: `accepts` returns a positive priority only when `input.host.state.gitRepo` is set (the file lives in a git repo), otherwise `-1`. Because the `SwitchWidget` (`PageToolbarView.ts`) also subscribes to `host.state`, the "Git Diff" switch appears the moment async git detection lands on the shared host (see [state-management.md](state-management.md#host-centric-git-detection)). This is how every text editor inherits the File Diff switch with zero per-editor code.
