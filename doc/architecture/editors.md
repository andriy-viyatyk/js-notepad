...
A board is not limited to its main iframe — it can contribute one or more **secondary views**, each a second `board://` iframe rendered in its own sidebar panel and wired to the same board model as the main view. This is what lets a board reproduce a built-in editor's main-view + sidebar-panel shape (the Todo list + its Lists/Tags panel, for example). Available to **every** board — plain, custom-editor, and content-host alike — because it lives on the base `BoardEditorModel`.
