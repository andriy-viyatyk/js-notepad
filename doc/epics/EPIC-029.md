...
| US-600-a | Links — always-on `SecondaryViews`, drop in-view panels, unify Category click | 1b | Panels render only via `SecondaryViews`; sidebar mandatory-open (`PageModel.sidebarMandatory`); Categories always categories+links; unified click (category→filter/promote, link→open); Tags/Hostnames parity (filter-only select); delete `leftPanelWidth`. **Prereq for US-601.** Interim: browser loses its Link panels until US-601. |
| US-601 | Browser adopts `SecondaryViews` in its empty page + bookmarks drawer | 2 | [Investigated + detailed](../tasks/US-601-browser-secondaryviews/README.md). `BrowserPanelHost` (`sidebarMandatory:true`, `mainEditorInstance→linkEditor`⇒`isMain:true`) on `BrowserBookmarks` + mount `SecondaryViews` on **both** `BlankPageLinks` and `BookmarksDrawer` to restore the panels US-600-a removed; centralize panel link-open via `LinkEditor.openLinkFromPanel` so `onLinkOpen` routes to the browser (Concern A); browser-state width persistence (Concern 4). **Must follow US-600-a before any release.** |
