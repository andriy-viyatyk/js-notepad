...
|------|-------|-----------|
| US-745 | **`board-manifest.json` — board identity file.** The canonical "this folder is a board" marker. v1 fields: `schemaVersion` + optional **descriptive metadata** (`name` else folder name, `description`, `author`, `repository`) — no behavior-driving fields. Icons stay a dropped `icon.*` file (US-744) — **no** manifest icon field. **Written on create** (added to `assets/board-template/` + `assets/demo-board/`, guaranteed by `ensureBoardManifest` in the create flow) and **required for enumeration** — `refreshBoards` lists a `boards/` subfolder only if it carries a manifest. Backfills the 11 existing local boards. **No trust field** (see C2). Reserve space for future `fileExtensions` (see Future). **[Investigated — doc ready.](../tasks/US-745-board-manifest/README.md)** | — |
| US-746 | **Boards anywhere.** Generalize a board's *location*: after US-745 a board is "any folder with a `board-manifest.json`", so discover/open boards **outside** `.persephone/boards/` too. Refactor `BoardEditorModel.refreshBoards()` (which after US-745 scans `<persephonePath>/boards` for manifest-bearing folders) and the private `createFromTemplate()` (today hardcodes `fpJoin(persephonePath, "boards", name)`) to accept an arbitrary board root; the public `createBoard(name)` / `createDemoBoard(name)` gain an optional `dir` so US-750 can target any path. **Extend `BoardEditorModel` state + the open path to accept a direct board-root path** (not only `.persephone` + name) — required by US-748's open-by-link. `.persephone/boards/` stays as the default grouping for project boards. **[Investigated — doc ready.](../tasks/US-746-boards-anywhere/README.md)** | US-745 |
| US-747 | **Trust at board level.** Move the gate from per-`.persephone`-project to per-board. New trusted-**boards** registry module (e.g. `board-trust.ts` + data file `trustedBoards.txt`) — path-keyed, in `<userData>/persephone/data/`, modeled on `project-trust.ts`'s `prepareDataFile`/`saveDataFile`; NOT a manifest field (C2). Decide whether `project-trust.ts` is retired or repurposed. Per-board "Trust board" dialog (RCE wording from US-721). Project trust redesigned as a **"Trust all boards in this project"** bulk action — exposed via the Explorer `.persephone`-node context menu (sibling of the existing "Create .persephone project" item); adds current boards to the registry, no standing grant for future ones (C4). Refactor `BoardEditorView.tsx` gate, `UntrustedProjectView`, and the create-flow auto-trust in `ExplorerSecondaryView`. **[Investigated — doc ready.](../tasks/US-747-board-level-trust/README.md)** | US-745 |
| US-748 | **Open-a-board link scheme.** New `persephone-board://` scheme (decided, C1 — **NOT** `board://`): encode/decode helper (`src/renderer/content/*-link.ts`) + a Layer-1 parser in `parsers.ts` (parser-only, like `persephone-folder://`; **no** new resolver presumed), `target: "board-view"`. It is an **in-app link scheme parsed in the renderer — NOT an Electron custom protocol** (contrast `board://`, a real protocol handler in `src/main/board-protocol-service.ts`); do not register it with `protocol.handle`. Opens a board by path through `openRawLink`, with optional forwarded params (e.g. `filePath`) carried on `ILinkData`. Models `persephone-folder://`. **[Investigated — doc ready.](../tasks/US-748-open-board-link-scheme/README.md)** | US-745, US-746 |
| US-749 | **Explorer "open board" row button.** On a `board-manifest.json` row in the Explorer file tree, add a **right-edge trailing icon button** (`BoardIcon`, "Open Board" tooltip). The row's normal click still opens the JSON in Monaco; the **button** opens the board (or shows the trust dialog if untrusted). Carve-time work = a per-row trailing action slot in the tree-row renderer — **not** folder-click interception (C7). **[Investigated — doc ready.](../tasks/US-749-explorer-open-board-button/README.md)** | US-745, US-748 |
| US-750 | **Board lifecycle: MCP tools + app API + agent guide.** Add first-class **`create_board`** / **`open_board`** MCP tools, plus `app.boards.createBoard`/`createDemoBoard`/`openBoard` and a generic `app.openRawLink(href)` on the renderer `app` API, so an agent creates a board at a **user-specified path** (auto-trusted, C5), opens it via a `persephone-board://` link (US-748), and develops it without user clicks. (C750-3/C750-4 revised after MCP testing — boards warranted first-class tools + an `openBoard` method.) Advertises the capability in the MCP instruction + adds a new agent-facing **`boards` guide** (`read_guide("boards")`). **[Implemented.](../tasks/US-750-board-lifecycle-api-mcp/README.md)** | US-748 |
| US-751 | **Sidebar "Tools & Editors" boards tab + board management.** Redesign the "All Editors & Tools" subsection into **two tabs** — *Editors* (today's `getCreatableItems()` list) and *Boards* (trusted boards as a folder-grouped **tree**). The *Boards* tab is the single management surface: **open** a board (US-748), **pin/unpin** alongside built-in editors (extend the `pinned-editors` setting), and **Remove board** via context menu — where *remove ≡ untrust* (edits the trusted-boards registry from US-747). Source = the trusted-boards registry (C3: *trusted ≡ registered*). **Pin-id format:** resolved by overloading the existing `pinned-editors` key — boards stored as `"board:<root>"`, editors as bare ids, decoded via a `PinnedRef` wrapper (`pinned-items.ts`), so pins are one unified ordered list. Pinned region is lifted above a `SegmentedControl` (Editors / Custom Boards); the *Boards* tab (`TrustedBoardsList.tsx`) groups trusted boards by folder, opens via the raw `persephone-board://` link, and **Remove ≡ untrust** (`boardTrust.untrust`, folder untouched). **[Implemented.](../tasks/US-751-tools-editors-boards-tab/README.md)** | US-745, US-746, US-747 |
