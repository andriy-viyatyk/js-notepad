...
| [US-838](../tasks/US-838-filepath-into-board/README.md) | **filePath into the board.** Add `ILinkData.filePath` + `BoardEditorState.filePath` + `currentFilePath()`; forward via `persephone-board://` → `sourceLink`/state → `BoardPortInitMsg` → async `persephone.getFilePath()` (shim; no property). _(carved 2026-07-14; `switchFrom`/old-host extraction reassigned to the crux, US-839)_ | US-837 |
| [US-839](../tasks/US-839-resolution-switch-integration/README.md) | **Resolution + switch integration (crux).** Separate merged resolver `resolveEditorIdForFile` (NOT baked into `editorRegistry.resolveId` — would poison `newTextFileModel`/`resolvers`); `buildEditorById` `board-editor:` branch (→ BoardEditorModel + filePath via `initFromBoardRoot(root, filePath)`); switch-widget board merge + reactive subscribe + label (single point, not the 16 editors); dynamic `BoardEditorModel.editorId` + `getRestoreData` pins `"board-view"` for persistence + `filePath`/`findCompatibleEditors` overrides; `switchMainEditor` board-boundary branch rebuilding via `createEditorFromFile` (no shared host) + `confirmRelease()`/abort-on-cancel (CE4); broaden MCP/automation board detection via `isBoardEditorId`; local-file gate `isPlainLocalPath` (CE4); eager `customEditorRegistry.ensureInitialized()` at bootstrap. See design note #3 (a–e). _(carved 2026-07-14)_ | US-838 |
| [US-840](../tasks/US-840-drawio-viewer-board/README.md) | **DrawIO viewer board (US-454).** First real custom-editor board for `.drawio`; retires the built-in US-454 plan. Proves the epic end-to-end. _(carved 2026-07-14; board authored **outside** the repo in `C:\projects\persephone-boards\drawio-viewer`, destined for its own git repo — the plumbing is already live, so this is pure board authoring)_ | US-839 |
...
### 2026-07-14 — US-840 implemented (DrawIO viewer board — proving ground)
- Built the [DrawIO viewer board](../tasks/US-840-drawio-viewer-board/README.md) in `C:\projects\persephone-boards\drawio-viewer` (external repo): manifest `fileMasks:["*.drawio"]` / `editorPriority:100` / `editorName:"DrawIO"`, vendored `viewer-static.min.js` v30.3.8 (offline), `getFilePath → readFile → GraphViewer`, own multi-page tab bar (GraphViewer's toolbar needs CSP-forbidden remote assets).
- **The proving ground surfaced two US-839 plumbing gaps** (never exercised until a real file-associated board existed) — both fixed, both part of the epic close-out review:
  1. **Render path** (`RenderEditor.tsx`) did a static `editorRegistry.getById(board-editor:<root>)` → threw `No editor registered` + perpetual spinner. Fixed by mapping `board-editor:<root>` → the `board-view` view module.
  2. **No switch-back UI from a board** — a board renders `BoardToolbar`, not `PageToolbar`, so the `SwitchWidget` never showed while on the board. Fixed by exporting `SwitchWidget` (`PageToolbar.tsx`) and rendering it in `BoardToolbar.tsx`.
- **CE-verified end-to-end:** resolution (direct + openRawLink), single/multi/compressed render, board↔Monaco switch (raw XML, `confirmRelease`), empty state for a plain board, offline (clean `ui.log`). This validates the full manifest → registry → resolution → switch → `getFilePath` path and retires the built-in US-454 plan.

### 2026-07-14 — US-839 carved (resolution + switch integration, the crux)
- Fourth task carved: [US-839](../tasks/US-839-resolution-switch-integration/README.md). Verified the full live call graph (`newEditorModel` → `resolveId` → `buildEditorById`; `SwitchWidget` → `findCompatibleEditors`; `switchMainEditor` → `switchFrom`) against source.
- **Merge lives OUTSIDE `editorRegistry` (refines CE6).** A separate `resolveEditorIdForFile` helper reads both registries — because `newTextFileModel` (`TextEditorModel.ts:441`) and `resolvers.ts` call `editorRegistry.resolveId` internally; baking the board merge into `resolveId` would stuff a `board-editor:<root>` id into `state.editor` and crash `attachEditorToPage`. Confirmed every real-path `newTextFileModel(filePath)` is inside the (guarded) `buildEditorById`, so the localized helper is sufficient.
- **Switch merge is a SINGLE point, not 16 editors.** The 16 text editors share one `findCompatibleEditors` body delegating to `findEditorsAccepting` (which skips non-host defs). Rather than edit all 16, the board options are appended in `SwitchWidget` itself; only `BoardEditorModel.findCompatibleEditors` is added (returns `[builtinId, self]` so the widget shows while on the board).
- **Two persistence/identity hazards found + resolved:** (1) a dynamic `editorId` would be persisted by `getRestoreData` and then miss `NO_HOST_EDITOR_IDS`/the zombie guard → board dropped on restore + cross-window move → override `getRestoreData` to pin `"board-view"` (virtual id re-derived from persisted `state.filePath`). (2) MCP/automation board detection keys on `editorId === "board-view"` at 4 sites (`commands.ts:44`, `mcp-handler.ts:210/237/693`) → broaden via `isBoardEditorId` so US-840's DrawIO board stays automatable.
- **Board-boundary switch** rebuilds through `createEditorFromFile` (public, on `pagesModel.lifecycle`) in BOTH directions — a board has no CONTENT_HOST_TRAIT for `switchFrom` to inherit; CE4 dispose-and-rebuild + `confirmRelease()`/abort-on-cancel.
- **CC2 resolved (user): route openRawLink too.** The universal `openRawLink` flow (Link editor, another board, MCP, drag-drop) must land a `.drawio` in the registered board. Implemented as a one-line target swap in the Layer 2 file resolver (`resolvers.ts`) reusing `resolveEditorIdForFile` — **no URL rewrite** (target becomes `board-editor:<root>`, the real file path rides the normal `openFile` arg), so the per-board-singleton `matchesNavigationTarget` (scoped to `target === "board-view"`) doesn't collapse distinct files. Plain `persephone-board://` opens are untouched.

### 2026-07-14 — US-838 carved (filePath into the board)
- Third task carved: [US-838](../tasks/US-838-filepath-into-board/README.md) — the `filePath` plumbing. `busy` (US-799) is the exact delivery template: per-open value carried to the board at the one-time port handshake.
- **Delivery:** `ILinkData.filePath` (persisted, free field — no collision) → `state.sourceLink.filePath` (openRawLink path) **or** `state.filePath` via `initFromBoardRoot(root, filePath)` (switch path, US-839) → `BoardEditorModel.currentFilePath()` merges both → `BoardPortInitMsg.filePath` → shim `persephone.getFilePath()`. URL stays a pure board id (filePath rides `ILinkData`, never the link — would break `matchesNavigationTarget`).
- **`switchFrom`/old-host extraction reassigned to US-839** (the crux owns the switch machinery that populates `filePath`); US-838 delivers filePath *given the model has one*.
- **CE1 decided (user): async method, no property.** The board surface is a single `persephone.getFilePath(): Promise<string | undefined>` that resolves at the handshake (mirrors `getBoardBusy`) — no `persephone.filePath` property. Board authors `await` it and rely on it regardless of timing; a plain board resolves to `undefined`.
- Task numbers firmed: US-838 filePath, **US-839 crux**, **US-840 DrawIO** (corrects an earlier slip that called the crux "US-838").
