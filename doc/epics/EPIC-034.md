...
| US-728 | Demo board — self-documenting showcase **+ dogfood** (evolved from the Test board): demonstrates the full `persephone` surface (`execute`, integration tier, **theme + tokens**), proves the full loop, and embeds a **recommended-components catalog with links to the `boards-assets/` skin CSS** (skin links pending US-727). **Bundles the prepared demo as `assets/demo-board/`** and offers it from the Board editor — **empty-state "Create Demo board" button + "+ New board" `SplitButton` dropdown** (prompts for a name, copies the bundled demo; no project-creation dialog). **[Investigated — doc ready.](../tasks/US-728-demo-board/README.md)** |
...
| US-734 | Recommended component — **Chart.js** (charts/dashboards). Vendor + JS theme adapter (colors from `persephone.theme.vars`, re-applied on `onThemeChange`) + manifest entry + demo. Extends the US-727 skins model. |
| US-735 | Recommended component — **Flatpickr** (date / time / range picker). Vendor + `--p-*` CSS skin + manifest entry + demo. |
| US-736 | Recommended component — **Tom Select** (rich select / tags / autocomplete). Vendor + `--p-*` CSS skin + manifest entry + demo. |
| US-737 | Recommended component — **marked + highlight.js** (markdown render + code highlighting). Vendor + `--p-*` highlight theme (maps to the app's editor token colors) + manifest entry + demo. |
| US-738 | Recommended component — **Mermaid** (diagrams). Vendor (IIFE bundle) + JS `themeVariables` fed from `persephone.theme` + manifest entry + demo. |
| US-739 | Recommended component — **Split.js** (resizable layout panes). Vendor + `--p-*` CSS skin (gutter) + manifest entry + demo. |
...
### Adoption workflow (per-component tasks — proven on US-727)

How to take a recommended-component task (US-734…US-739, and any future one) from placeholder to a shipped skin. This is the **autonomous loop** US-727 used — the next agent should follow it.

**1. Dedicated demo board per component.** Create a working board at `.persephone/boards/<Component>/` (this path is **gitignored** — a local working artifact, never committed). Scaffold from the template, then:
   - **Vendor the library locally** into `lib/` (download its UMD/IIFE JS + any CSS) — the board CSP forbids remote network, so reference it with relative paths, never a CDN. Pin and record the exact version for the skin stamp.
   - Build a small `index.html` + `app.js` that **exercises the component's main features**, and where it makes sense **drive its data through `persephone.execute()`** (a `scripts/` backend) so the board also dogfoods the channel. The board chrome itself is styled with `--p-*`.

**2. Author the skin/adapter against `--p-*`.** Two shapes, depending on how the component colors itself:
   - **CSS-styled components** (Flatpickr, Tom Select, highlight.js, Split.js, …): a `<component>.css` skin written *entirely* against `--p-*` (with literal fallbacks), version-stamped header (`/* persephone skin · <pkg>@<ver> · tuned <YYYY-MM> */`), commented per block. Use `color-mix(in srgb, var(--p-accent) N%, …)` for hover/selection tints so they adapt to *both* themes instead of hardcoding theme-specific colors. Link it **after** the component's own CSS.
   - **JS-colored components** (Chart.js, Mermaid, …): a small JS theme-adapter that builds the component's color options from the `--p-*` palette and **re-applies on `persephone.onThemeChange`**. Take the palette as a parameter: at init pass `persephone.theme` (load snapshot), and on a switch pass the palette **delivered to the `onThemeChange` callback** (or call `persephone.getTheme()`) — *not* a cached `persephone.theme`, which is a snapshot and goes stale after a switch (US-734 fix). Document the CSS-vs-JS boundary. Reference adapter: `boards-assets/chart-theme.js`.

**3. Autonomous style review via the persephone MCP.** With the board open in Persephone (the user opens it; an untrusted project won't render), verify *without eyeballing alone*:
   - `list_pages` → find the board (`editor: "board-view"`, with its `selectedBoard`) and its `pageId`.
   - `browser_take_screenshot` for a visual pass, **and** `browser_evaluate` to confirm every color **traces to a `--p-*` value** (equals the inherited `--p-*`, *not* the skin's literal fallback and *not* the component's shipped default). For **CSS** skins, probe `getComputedStyle` of each element; for **JS-colored** components (canvas), read the live instances' resolved options (e.g. `Chart.getChart(canvas)` → dataset/scale colors, `Chart.defaults`) and compare to the palette. Exercise every interaction state (open dropdowns/menus, selections, sorted/grouped/empty states, popups rendered at `document.body`).
   - **Verify both dark and light themes.** Switch the **app** theme for real (the board follows): in `execute_script`, `const { applyTheme } = await import("../theme/themes/index.ts"); applyTheme(id); app.settings.set("theme", id);` — **applyTheme first, then `set`**, because the board push (`BoardWebview` `settings.onChanged`) reads the *currently-applied* DOM theme via `computeBoardThemePalette()`. Theme ids: `default-dark`, `light-modern`, `monokai`, `solarized-dark/-light`, `quiet-light`, `abyss`, `red`, `tomorrow-night-blue`. **Restore the user's theme when done.**
   - ⚠️ **Caution (learned on US-727):** the `--p-*` vars are injected by the preload as **inline styles on `<html>`**. Do **not** "test light theme" by `documentElement.style.setProperty`/`removeProperty` — `removeProperty` *deletes the preload-injected vars* and the whole board falls back to literals (looks like a skin bug, isn't). A plain `location.reload()` restores the preload vars. **`persephone.theme` is a load snapshot (stale after a switch)** — read live colors from the `onThemeChange` argument or `getTheme()`. Always leave the board clean (close popups, reload if you disturbed state).

**4. Delegate the bulky parts.** This loop parallelizes well (US-727 did): delegate to cheaper subagents — (a) **vendor the lib**, (b) **enumerate the component's themeable selectors / theming API**, (c) **resolve the `--p-*` palette** to hex if needed, and (d) a **background exhaustive interaction+probe sweep** (read-only MCP) that returns a discrepancy list. Keep the skin authoring + visual judgment on the main thread.

**5. Promote + manifest.** Copy the proven skin to `boards-assets/<component>.css` (frozen, stamped) and add a `boards-assets/manifest.json` entry: purpose, tested version, vendor URLs, and the **CSS-vs-JS note** (what falls on the JS side). `boards-assets/` is the only committed output; the demo board stays local. The demo content can later seed US-728's recommended-components catalog.
...
### 2026-06-20 — US-728 carved (Demo board: bundle + create entry points)
- **US-728 task doc written** (`doc/tasks/US-728-demo-board/README.md`). The demo *content* is already built — the working `.persephone/boards/Demo Prototype/` board (Overview / Theming / Capabilities / Build Guide / Debugging; `--p-*` theming; MCP-debugging docs). This task **snapshots** it to `assets/demo-board/` and wires creation: empty-state "Create Demo board" button + a `SplitButton` on "+ New board" whose dropdown creates a demo (prompts for a name, copies the bundled demo).
- **Mechanics:** generalize `scaffoldBoard(destDir, template = "board-template")`; add `createDemoBoard` (shares a private `createFromTemplate`); `SplitButton` already wraps `WithMenu` so no extra wiring. `assets/` auto-ships via `forge.config.ts` `extraResource` — no build-config change.
- **Skin links** in the demo's recommended-components catalog stay "planned" until **US-727** lands.
- **`assets/demo-board/` is the single source of truth (user).** Future demo edits go directly there; the local `.persephone/boards/Demo Prototype/` is the migration source only and gets deleted after bundling — testing is via a throwaway "Create Demo board" instance. No sync mechanism (C-A resolved).
...
### 2026-06-20 — US-727 implemented + recommended-components expansion carved
- **US-727 implemented** (skin proven, not yet `/review`'d — epic deferred-review model). Vendored Tabulator **6.5.1** into a local test board, built an `execute()`-driven grid page, and authored `boards-assets/tabulator.css` — a skin written entirely against the `--p-*` contract (theme-adaptive hover/selection via `color-mix()`), version-stamped, commented per block. Published `boards-assets/` (skin + `manifest.json` + `README.md`) as the recommended-components home (not bundled — epic C7). Verified live via the persephone MCP across **dark + light** and an exhaustive interaction sweep (filter dropdowns, context menu, range selection, sort arrows, group/calc rows, pagination, placeholder, toggle, alert, resize guides) — **zero off-theme colors**. The CSS-vs-JS split is documented (progress-bar/formatter colors come from `persephone.theme.vars`).
- **Decision (user, 2026-06-20): expand the recommended-components set** beyond the grid, one task per component (each follows the US-727 model — vendor → skin/JS-adapter → manifest entry → demo). Carved **6 placeholders** for distinct purposes: **US-734 Chart.js** (charts), **US-735 Flatpickr** (date/time picker), **US-736 Tom Select** (rich select/tags), **US-737 marked + highlight.js** (markdown + code), **US-738 Mermaid** (diagrams), **US-739 Split.js** (resizable layout). All vanilla / single-`<script>` / vendorable-offline. No task docs yet — each is investigated before its implementation. Skipped by design: toast libs (`notify()` covers it), modal libs (native `<dialog>`), and another grid (Tabulator owns it).

### 2026-06-20 — US-734 implemented (Chart.js) + US-725 theme-contract fixes
- **US-734 implemented** ([task doc](../tasks/US-734-chartjs-skin/README.md); not yet `/review`'d — deferred-review model). First **JS-adapter** recommended component (canvas, no CSS to skin): vendored Chart.js **4.4.6** into a local demo board (line/bar/doughnut/radar, data via `execute()`), authored `boards-assets/chart-theme.js` (`window.PersephoneChartTheme` — pushes the `--p-*` palette into `Chart.defaults` + dataset colors), and added the `chartjs` manifest entry + README CSS-skin-vs-JS-adapter workflow. Verified live via the persephone MCP across **dark + light** — every dataset/grid/tick/tooltip color traces to a `--p-*` token; live recolor on switch confirmed via `onThemeChange`.
- **US-725 theme-contract bugs found via the MCP review and fixed (infra, not the skin):**
  1. **`persephone.theme` was stale after an in-session theme switch** — the guest's `contextIsolation` makes `contextBridge` copy the `theme`/`tokens` getters once at expose time; only `onThemeChange` (a proxied function) was live. Added live **`getTheme()`/`getTokens()`** functions; `persephone.theme` is now documented as a load-time snapshot.
  2. **Guest reload painted the registration-time theme** — `getContext` returned the palette stored in `sessionToDesign` at registration, never refreshed. Added an **`updateBoardTheme`** IPC that refreshes the stored palette for all live sessions on every switch, so a reloaded guest reads the current theme.
  - Touched preload/bridge/main/IPC + the authoring guide (`assets/board-template/CLAUDE.md`). **Requires an app restart** to take effect. The recorded adoption playbook's "reads `persephone.theme.vars` … re-applies on `onThemeChange`" guidance is superseded by "use the `onThemeChange` argument or `getTheme()` for live re-theming."

### 2026-06-20 — US-735 implemented (Flatpickr)
- **US-735 implemented** ([task doc](../tasks/US-735-flatpickr-skin/README.md); not yet `/review`'d — deferred-review model). Second **CSS-skin** recommended component (after Tabulator): vendored Flatpickr **4.6.13** into a local demo board (single / date+time / range / time-only / month-dropdown / inline+week-numbers, inline config via `execute()`), authored `boards-assets/flatpickr.css` against `--p-*`, and added the `flatpickr` manifest entry + README row. Verified live via the persephone MCP across **dark + light** — every calendar surface (container, day-cell states, today=accent outline, range endpoints/in-range tint, time picker, week numbers) traces to a `--p-*` token; flatpickr's hardcoded accents (`#569ff7`/`#959ea9`/`#f64747`) are replaced with `--p-accent`. Pure CSS skin → theme switch re-tints with **no JS** (confirmed live, no reload). Reaffirms the playbook: flatpickr does **not** style the text `<input>` — the board owns it.

### 2026-06-20 — US-736 implemented (Tom Select)
- **US-736 implemented** ([task doc](../tasks/US-736-tom-select-skin/README.md); not yet `/review`'d — deferred-review model). Third **CSS-skin** recommended component: vendored Tom Select **2.4.3** (`complete` build, all plugins) into a local demo board (single / multi+remove / free-tag create / grouped picker with options via `execute()`), authored `boards-assets/tom-select.css` against `--p-*`, and added the `tom-select` manifest entry + README row. Verified live via the persephone MCP across **dark + light** — control box (mirrors the board's inputs), tag chips, dropdown panel, optgroup headers, the keyboard-`.active` option (solid accent), and the match `.highlight` all trace to `--p-*`; Tom Select's hardcoded accents (`#f5fafd`/`#495c68` active option, `rgba(125,168,208,0.2)` highlight) are replaced with `--p-accent`. Pure CSS skin → theme switch re-tints with **no JS** (confirmed live, no reload).

### 2026-06-20 — US-737 implemented (marked + highlight.js)
- **US-737 implemented** ([task doc](../tasks/US-737-markdown-skin/README.md); not yet `/review`'d — deferred-review model). Fourth **CSS-skin** task, and the first **two-library** component: **marked 15.0.12** (markdown → HTML; ships no CSS) + **highlight.js 11.11.1** (`@highlightjs/cdn-assets` common bundle; ships only class names, no colors). One combined skin `boards-assets/markdown.css` with two parts — (1) `.markdown-body` prose typography and (2) a highlight.js token theme — both against `--p-*`. Demo board loads its markdown over `execute()` (`scripts/sample.js` reads `content.md`), renders with `marked.parse()`, highlights each `pre code` with `hljs.highlightElement`. Added the `markdown` manifest entry (two-library vendor + token-map note) + README row.
- **New insight — folding a syntax palette onto `--p-*`:** `--p-*` has no syntax colors, so highlight.js's ~8 default hues are mapped onto the semantic tokens by role (keyword→accent, string→success, number→warning, title/attr→link, comment→muted, deletion→error, plain→text). This works in **both** themes because the theme system already keeps each semantic color legible on its own background — the light-theme risk (amber/green washing out on a near-white code panel) was specifically checked: light-modern's `--p-warning` is a *dark amber* (`rgb(154,103,0)`), not an invisible yellow. Probe-confirmed in **light** (where fallbacks ≠ theme) that every token traces to its `--p-*`, not the skin fallback. Reusable pattern for any future code-coloring component. Pure CSS skin → theme switch re-tints document + syntax with **no JS** (confirmed live, no reload).

### 2026-06-20 — US-738 implemented (Mermaid)
- **US-738 implemented** ([task doc](../tasks/US-738-mermaid-skin/README.md); not yet `/review`'d — deferred-review model). Second **JS-adapter** recommended component (after Chart.js): **Mermaid 11.15.0** renders diagrams to SVG colored from JS (ships no stylesheet), so the skin is `boards-assets/mermaid-theme.js` — `window.PersephoneMermaidTheme.config(theme)` builds Mermaid's `themeVariables` (on the customizable `theme:'base'`) from the `--p-*` palette: node fill/border/text, edges, clusters, the full sequence set (notes=`--p-warning` tint, activation=`--p-accent`), pie slices (`pie1..12` from the palette), and the gantt set. Demo board loads six diagram specs over `execute()` (`scripts/diagrams.js`) and renders each with `await mermaid.render(...)`. Added the `mermaid` manifest entry (JS-adapter + re-render note) + README row. Verified live across **dark + light**; probe in light confirmed node fill=`--p-panel`, border=`--p-border`, edges=`--p-text-muted`, pie=`--p-accent`/`--p-success`, note=`--p-warning` — all trace to `--p-*`.
- **New insight — Mermaid is NOT a live-update library (re-render, don't just re-init):** the three skin shapes now differ in how a theme switch is applied. A **CSS skin** re-tints with no JS. **Chart.js** picks up new `Chart.defaults` on the next `.update()` of a *live* chart instance. **Mermaid bakes colors into the SVG at render time** — `initialize()` only affects future renders, so a theme switch must **re-initialize AND re-render every diagram from its source**. The board keeps the diagram specs and, in `onThemeChange`, calls `mermaid.initialize(config(theme))` then re-runs `mermaid.render(...)` per diagram (fresh unique ids). Recorded in the manifest `cssVsJs` note so future Mermaid boards get the re-render right. Also: Mermaid derives unset diagram-family vars to off-palette greys (the gantt bars were grey until the gantt `themeVariables` group was added explicitly) — set the family-specific vars for any diagram type the board actually uses.
