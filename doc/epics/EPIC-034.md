...
**2. Author the skin/adapter against `--p-*`.** Two shapes, depending on how the component colors itself:
   ...
   - **JS-colored components** (Chart.js, Mermaid, …): a small JS theme-adapter that reads `persephone.theme.vars` to build the component's color options, and **re-applies on `persephone.onThemeChange`**. Document the CSS-vs-JS boundary.
...
**3. Autonomous style review via the persephone MCP.** With the board open in Persephone (the user opens it; an untrusted project won't render), verify *without eyeballing alone*:
   ...
   - `browser_take_screenshot` for a visual pass, **and** `browser_evaluate` to **probe `getComputedStyle`** of each element and confirm every color **traces to a `--p-*` value** (equals the element's inherited `--p-*`, *not* the skin's literal fallback and *not* the component's shipped default). Exercise every interaction state (open dropdowns/menus, selections, sorted/grouped/empty states, popups rendered at `document.body`).
   - **Verify both dark and light themes.**
   - ⚠️ **Caution (learned on US-727):** the `--p-*` vars are injected by the preload as **inline styles on `<html>`**. Do **not** "test light theme" by `documentElement.style.setProperty`/`removeProperty` — `removeProperty` *deletes the preload-injected vars* and the whole board falls back to literals (looks like a skin bug, isn't). To check light theme, either switch the app theme for real, or save/restore the exact inline values; a plain `location.reload()` restores the preload vars. Always leave the board clean (close popups, reload if you disturbed state).
