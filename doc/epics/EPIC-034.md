...
- **Live in-app board testing via MCP** — adapt Persephone's existing browser-automation MCP (`browser_*`, Playwright-compatible) to target a board's webview, so an agent can snapshot/interact/debug the real board end-to-end. Persephone already has the MCP + automation pieces. (Own epic/task.) *(This — not a mock dev-shim, which was cut — is the planned richer testing story.)*
...
- **Offered on project creation via a dialog (NOT silent auto-create).** On `.persephone` project init, prompt **"Create a Demo board?"** with **Create (default)** / **Don't create**, plus a **"Do not show this dialog again"** checkbox. If checked → future project creation shows no dialog and creates no Demo board. Persist as an app preference (e.g. settings `createDemoBoardPrompt: "ask" | "never"`). Scaffolding ties into US-726.
- **Depends on:** US-725 (theme/tokens — done), US-726 (template + scaffold + the prompt dialog), US-727 (skins to link). Stays a placeholder; full Goal→Plan→Concerns doc written when its turn comes (after US-726/727 land), seeded from the current Test board panels.
