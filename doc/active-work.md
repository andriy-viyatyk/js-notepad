...
- *(no epic)*
  - [ ] US-1173: **interactive verification pass for EPIC-073's converted surfaces.** Every one of
    ...
    surfaces. **Added by EPIC-074**, which deferred four checks by decision: (a) **a theme switch
...
because it needs no ground truth prepared in advance.** **E15 is complete as [EPIC-073](epics/completed.md) (2026-08-28)** — Epic E is finished; see the close record in the roadmap. It was scoped as follows: — the last React editor: the five remaining React editor bodies (`graph`, `rest-client`, `env-vars`, `file-diff`, `draw`, 383 of the renderer's 403 JSX markers), the 21-face `mountVanilla` layer that only those bodies render, and the residual React paths inside `PopoverView`/`DialogView`. It is Epic E's last epic. Re-measuring E14's handoff confirmed more than it corrected — the five marker counts and `graph` as `highlight.ts`'s last React consumer all verified exactly — but it found the face count is 21 not 19, `EditorErrorBoundary.tsx` **cannot** die (storybook renders through it), the `SlotText` sweep is not De-React work at all (both aliases include `React.ReactNode`), and the handoff omitted what actually gates Epic F: ~260 `React.*` **type** references across 80 files that contain no React runtime. Next free epic number: **EPIC-074**; next free task number: **US-1173**. **Epic F is the only epic left**, and its opening line changed: `react`/`react-dom` stay, scoped to the Excalidraw editor (user decision 2026-08-27). Its real blocker is the ~70-file React **type** surface, and its ending is enforceable — an ESLint rule confining `react` imports to `editors/draw/**`, with 84 importers as the baseline. **Epic F is now scoped as [EPIC-074](epics/EPIC-074.md) (2026-08-28)** — *React confined*. Its fresh measurement **confirmed every number in E15's handoff** (84 / 14 / 70 / 3 roots, all five instrument cases green — the programme's first inherited baseline needing no re-baselining) and corrected three of its conclusions: nothing can be uninstalled by doing the type-surface work (Excalidraw's own `.d.ts` imports `react`, so `@types/react*` stay too — the type surface gates the **ESLint rule**, not a package removal); E15's closing statement 2 was **false as written**, because ten `.ts` files still match `return mountVanilla(` and a `.tsx`-only filter could not see them; and the seven "hook-exporting modules" are alive with only their nine hook entry points dead. **The programme ends with React installed, and that is the stated outcome rather than a shortfall.** **Epic F is complete as [EPIC-074](epics/completed.md) (2026-08-28) and the De-React programme is FINISHED** — `react`/`react-dom` are importable from exactly one directory, `editors/draw/`, enforced by a `no-restricted-imports` rule proven to fail on a `uikit/` import and pass on a draw one. Importers **85 → 2**, value users **16 → 2**, type-only **69 → 0**; the only `.tsx` left is `ExcalidrawIsland.tsx`. Its own largest finding was that the instrument counting React since Epic A was wrong three ways — quote style, no-import global namespace, and bare-identifier value use — two of them blocking, which is why the corrected baseline is 85/16/69 rather than the 84/14/70 published at scoping. No epics remain.
