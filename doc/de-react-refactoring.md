...
> **Status:** Implemented in [EPIC-075](epics/EPIC-075.md) — US-1192, US-1193, and US-1194.
> `memo()` / `IMemo` remain as the one cached computation primitive and are deferred to Epic B.

**Delivered contract.** `TComponentModel` now exposes `props`, `setProps`, `init`, `dispose`, and
`memo`; the effect runtime, React-internal fields, and driver's throw branch are gone. The settings
sections use `createComponentModelDriver`, and each model owns its dependency gates in `setProps()`
behind an initialization guard. `isFirstUse` and `oldProps` readers now use named local previous
values, so views no longer call model internals.

The default-browser model was also folded into its view because it had no meaningful independent
stateful behavior. The resulting contract is a vanilla lifecycle: the driver pumps initial props,
`mount()` calls `init()` once, later `update()` calls `setProps()`, and `dispose()` releases model
resources.
...
> **Status:** Implemented in [EPIC-075](epics/EPIC-075.md) — US-1195 and US-1197.

The baseline counts and leak examples in the original proposal below predate delivery. The current
contract is one `() => void` subscription disposer: `ownSubscription()` returns an early-release
handle, and every census site is owned or has an explicit exception rationale.
...
**Original proposal (superseded by the delivered contract above).**
...
- `core/state/model.ts` no longer schedules a constructor-wide `postCreate` timer; the one consumer
  calls its post-create work explicitly. The remaining R8 timing items still need individual review.
...
- **23** other `setTimeout(...,0)` ordering hacks remain in editors — none carries a comment explaining the ordering constraint it encodes. Rule: every deliberate deferral must state what it waits for, or be replaced by an explicit call.
...
3. **Scheduling helpers instead of ad-hoc timers.** `core/utils/scheduling.ts` provides the live `Delayer`, `afterPaint`, and `focusAfterPaint` helpers. `Throttler` and `RunOnceScheduler` were not added without real callers; the remaining deferrals stay in R8 for individually verified conversions.
