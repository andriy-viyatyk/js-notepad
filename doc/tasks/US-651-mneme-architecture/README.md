...
5. **Reindex = cancellable background job (JobManager).** `wiki_reindex` starts a job that streams chunks to the embedder at *bulk* priority. The manager provides **progress** (processed/total → MCP progress notifications + `wiki_status` / `mneme://status` resource → Persephone progress bar), **cancellation** (cancel token / clean shutdown), **backpressure** (bounded producer→embedder queue so a huge folder doesn't balloon memory), and **single-flight per root** (a watcher event mid-run is coalesced/queued). Progress payload: `total` = file count (scanning) / docs to embed (embedding), `progress` = items processed, with the root + phase in the notification `message`.

> **Amended during US-659 implementation (2026-06-13):** MCP `progressToken`s are **client-supplied** — the server cannot mint `"reindex:{root}"`, it echoes the token the client put in the request `_meta` (present only if the client opted into progress). So: MCP progress uses the **client token**, and the root + phase (`scanning`/`embedding`/`done`/`cancelled`) go in the **`message`** field; the always-on, root-keyed progress snapshot (the `"reindex:{root}"` keying) lives in **`wiki_status.reindex`** (`{phase, processed, total}` per root), read off the `JobManager`. Cancellation rides the MCP request's cancel token (rmcp `RequestContext.ct` → `notifications/cancelled` / timeout); the watcher + deferred startup reconcile run as **coalesced** (not request-bound) jobs observable only via `wiki_status`.
