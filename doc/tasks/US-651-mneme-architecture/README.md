...
See [`mneme-architecture.mmd`](mneme-architecture.mmd) (open in Persephone's Mermaid viewer). Reproduced here:
...
flowchart TB
    ...
    subgraph mneme[" Mneme service — single self-contained Rust binary "]
        MCP["MCP server — sole interface<br/>(Streamable HTTP + stdio, bearer/OAuth)<br/>tools = actions · resources = content"]
...
- **Tools = actions (data plane):** `wiki_search` (query + filters: subtree, tags include/exclude, area, date range, mode, **`ext`** — defaults to `.md`; array or `".*"` to include other indexed types), `wiki_read`, `wiki_write`, `wiki_delete`, `wiki_tree`, `wiki_timeline`. Self-describing via JSON schema — discoverable by agents and testable in Persephone's MCP Inspector. All root-scoped tools take an **optional `root`** — defaults to the sole root; required for path tools when multiple roots exist; `wiki_search`/`wiki_timeline` default to all roots (merged) when omitted.
...
- **Embedding Engine** — wraps an `ort` session + tokenizer. Selects execution provider (DirectML → CPU fallback), batches inference, applies query/passage instruction prefixes, optional Matryoshka dim-truncation. Exposed behind an `Embedder` trait so a remote/Azure embedder can replace it without touching callers.
- **Search Engine** — builds the metadata filter as a SQL `WHERE` (subtree = path prefix, tags include/exclude, area, date range, and **`ext`** — defaults to `.md`), runs the **FTS5 BM25** query and the **sqlite-vec KNN** query over the filtered set, merges the two ranked lists with **RRF**, then resolves chunk hits to document results with snippets. Tag/date/area predicates apply only to `.md` (non-md files have no such metadata).
...
- **Model Provisioner** — on first run / model change, downloads the ONNX model + tokenizer from a known URL (e.g. the service's GitHub release) into a cache dir and verifies a checksum. FTS-only search works before the model is present.
...
sequenceDiagram
    ...
    SE->>SE: build metadata filter (subtree / tags / area / dates)
    par full-text
...
|---------|--------|
| Async runtime | `tokio` |
| MCP server (sole interface) | official Rust MCP SDK (`rmcp`) — tools + resources; stdio + Streamable HTTP transports (HTTP transport built on `axum`/`hyper`) |
...
| YAML frontmatter | `serde_yaml_ng` / `serde_norway` (serde_yaml is deprecated) |
...
- **Persephone renderer** consumes Mneme through the existing MCP client (`McpConnectionManager`, official `@modelcontextprotocol/sdk`):
  - **`MnemeProvider`** — a content-pipeline provider (like `FileProvider`) that **reads and writes** document text and **image/attachment bytes** over MCP (`wiki_read` / `resources/read`; `wiki_write` / `wiki_delete`). Persephone's existing editors open and save through it; images resolve to blob/data URLs;
  - **`MnemeTreeProvider`** — a tree provider (like `FileTreeProvider`) that renders the **category / document tree** from `wiki_tree`, shown with Persephone's Explorer-style tree component;
...
- **AI agents** connect to the **same MCP server** — directly, or surfaced through Persephone's own MCP server. Identical tool/resource surface; no proxy or second contract.
...
- [x] **Multiple-roots addressing — RESOLVED (2026-06-13):** a separate **optional `root`** tool parameter (not a path prefix). Defaults to the sole root when one is registered; required for path-addressed tools (`wiki_read`/`write`/`delete`/`tree`/`reindex`) when multiple roots exist; `wiki_search`/`wiki_timeline` default to all roots (merged, filters still apply) and accept `root` to scope. Resources stay explicit via `mneme://{root}/{path}`.
...
- [x] **Startup reconcile — RESOLVED (2026-06-13):** the reconcile runs as a **deferred background job** (~5 s after launch, configurable), **never blocking startup** — Mneme serves immediately against the persisted index, and the job brings it current shortly after (progress in the monitoring UI / `wiki_status`; the watcher catches anything that changes meanwhile). Off the startup path, a **full content-hash scan** is fine (correct + simple); an mtime+size fast-path is an optional later optimization only if the background scan's CPU/IO cost ever matters (e.g. battery).
- [x] **Multi-type / code indexing — RESOLVED (2026-06-13):** non-markdown extensions **can be configured** for indexing (D18 include allowlist). They are **plain-text search targets** — chunked by **size** (no headings), with **no YAML frontmatter**, so **no `tags`/`date`/`area`** (documented limitation: only `.md` carries those). `wiki_search` gains an **optional `ext`**: omitted → `.md` only (default); else an array of extensions (e.g. `[".ts", ".js"]`) or `".*"` for all indexed types. Path/subtree/category filters still apply to non-md; tag/date/area filters simply don't match them. The category tree stays markdown-document-oriented; code files surface via search.
